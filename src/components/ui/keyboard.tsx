...
const KEY_HINT_LABELS: Record<string, string[]> = {
  ...
  ShiftRight: ["SHIFT"],
  ...Object.fromEntries(
    Object.entries(KEY_DISPLAY_LABELS)
      .filter(([keyCode]) => keyCode.startsWith("Key"))
      .map(([keyCode, label]) => [keyCode, [label]]),
  ),
  BracketLeft: ["Х"],
  BracketRight: ["Ъ"],
  Semicolon: ["Ж"],
...
const normalizeHintLabel = (label: string) => {
  if (["—", "–", "‑", "−"].includes(label)) {
    return "-";
  }
...
const keyMatchesHint = (keyCode: string | undefined, hints: string[]) => {
  if (!keyCode || hints.length === 0) {
    return false;
  }

  const keyLabels = KEY_HINT_LABELS[keyCode] ?? [getKeyDisplayLabel(keyCode)];
  const normalizedKeyLabels = keyLabels.map(normalizeHintLabel);
  const normalizedHints = hints.map(normalizeHintLabel);

  return normalizedHints.some((hint) => normalizedKeyLabels.includes(hint));
};
...
export const Keypad = ({
  ...
  const hintProps = { currentKey, errorKey, pressedKey, targetKeys };

  return (
    <div className="h-full w-fit rounded-[7px] bg-neutral-100 p-1 shadow-sm ring-1 ring-black/5 shadow-black/5">
      {/* Function Row */}
      <Row>
        <Key
          keyCode="Escape"
          containerClassName="rounded-tl-[7px]"
          className="w-10 rounded-tl-[5px]"
        ...
        <Key containerClassName="rounded-tr-[7px]" className="rounded-tr-[5px]">
      ...
      {/* Number Row */}
      <Row>
        <Key keyCode="Backquote" {...hintProps}>
          <span>Ё</span>
          <span>ё</span>
        </Key>
        <Key keyCode="Digit1" {...hintProps}>
          <span>!</span>
          <span>1</span>
        </Key>
        <Key keyCode="Digit2" {...hintProps}>
          <span>"</span>
          <span>2</span>
        </Key>
        <Key keyCode="Digit3" {...hintProps}>
          <span>№</span>
          <span>3</span>
        </Key>
        <Key keyCode="Digit4" {...hintProps}>
          <span>;</span>
          <span>4</span>
        </Key>
        <Key keyCode="Digit5" {...hintProps}>
          <span>%</span>
          <span>5</span>
        </Key>
        <Key keyCode="Digit6" {...hintProps}>
          <span>:</span>
          <span>6</span>
        </Key>
        <Key keyCode="Digit7" {...hintProps}>
          <span>?</span>
          <span>7</span>
        </Key>
        <Key keyCode="Digit8" {...hintProps}>
          <span>*</span>
          <span>8</span>
        </Key>
        <Key keyCode="Digit9" {...hintProps}>
          <span>(</span>
          <span>9</span>
        </Key>
        <Key keyCode="Digit0" {...hintProps}>
          <span>)</span>
          <span>0</span>
        </Key>
        <Key keyCode="Minus" {...hintProps}>
          <span>_</span>
          <span>-</span>
        </Key>
        <Key keyCode="Equal" {...hintProps}>
          <span>+</span>
          <span>=</span>
        </Key>
        <Key
      ...
      <Row>
        ...
        {[
          ...
          <Key key={code} keyCode={`Key${code}`} {...hintProps}>
            {letter}
          </Key>
        ))}
        <Key keyCode="BracketLeft" {...hintProps}>
          Х
        </Key>
        <Key keyCode="BracketRight" {...hintProps}>
          Ъ
        </Key>
        <Key keyCode="Backslash" {...hintProps}>
          <span>{`|`}</span>
          <span>{`\\`}</span>
        </Key>
      </Row>
      ...
      <Row>
        ...
        <Key keyCode="Semicolon" {...hintProps}>
          Ж
        </Key>
        <Key keyCode="Quote" {...hintProps}>
          Э
        </Key>
        <Key
          keyCode="Enter"
      ...
      <Row>
        ...
        <Key keyCode="Comma" {...hintProps}>
          Б
        </Key>
        <Key keyCode="Period" {...hintProps}>
          Ю
        </Key>
        <Key keyCode="Slash" {...hintProps}>
          <span>,</span>
          <span>.</span>
        </Key>
        <Key
      ...
      <Row>
        <ModifierKey
          keyCode="Fn"
          {...hintProps}
          containerClassName="rounded-bl-[7px]"
          className="rounded-bl-[5px]"
        >
          <span>fn</span>
          <IconWorld className="h-[6px] w-[6px]" />
        </ModifierKey>
        <ModifierKey keyCode="ControlLeft" {...hintProps}>
          <IconChevronUp className="h-[6px] w-[6px]" />
          <span>control</span>
        </ModifierKey>
        <ModifierKey keyCode="AltLeft" {...hintProps}>
          <OptionKey className="h-[6px] w-[6px]" />
          <span>option</span>
        </ModifierKey>
        <ModifierKey keyCode="MetaLeft" className="w-8" {...hintProps}>
          <IconCommand className="h-[6px] w-[6px]" />
          <span>command</span>
        </ModifierKey>
        <Key keyCode="Space" className="w-[8.2rem]" {...hintProps} />
        <ModifierKey keyCode="MetaRight" className="w-8" {...hintProps}>
          <IconCommand className="h-[6px] w-[6px]" />
          <span>command</span>
        </ModifierKey>
        <ModifierKey keyCode="AltRight" {...hintProps}>
          <OptionKey className="h-[6px] w-[6px]" />
          <span>option</span>
        </ModifierKey>
        {/* Arrow Keys */}
        <div className="flex h-6 w-[4.9rem] items-center justify-end rounded-[4px] p-[0.5px]">
          <Key keyCode="ArrowLeft" className="h-6 w-6" {...hintProps}>
            <IconCaretLeftFilled className="h-[6px] w-[6px]" />
          </Key>
          <div className="flex flex-col">
            <Key keyCode="ArrowUp" className="h-3 w-6" {...hintProps}>
              <IconCaretUpFilled className="h-[6px] w-[6px]" />
            </Key>
            <Key keyCode="ArrowDown" className="h-3 w-6" {...hintProps}>
          ...
          <Key
            keyCode="ArrowRight"
            {...hintProps}
            containerClassName="rounded-br-[7px]"
            className="h-6 w-6 rounded-br-[5px]"
          >
...
const Key = ({
  ...
  const isPressed =
    Boolean(keyCode && pressedKeys.has(keyCode)) ||
    keyMatchesHint(keyCode, pressedKey ? [pressedKey] : []);
  const isTarget = keyMatchesHint(keyCode, targetKeys);
  const isCurrent = keyMatchesHint(keyCode, currentKey ? [currentKey] : []);
  const isError = keyMatchesHint(keyCode, errorKey ? [errorKey] : []);
  ...
  return (
    <div className={cn("rounded-[4px] p-[0.5px]", containerClassName)}>
      <button
        ...
        className={cn(
          "flex h-6 w-6 cursor-pointer items-center justify-center rounded-[3.5px] bg-gray-100 shadow-[0px_0px_1px_0px_rgba(0,0,0,0.5),0px_1px_1px_0px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(255,255,255,1)_inset] transition-[background-color,box-shadow] duration-75 ease-out",
          isTarget &&
            "bg-[#eee9fb] shadow-[0px_0px_1px_0px_rgba(112,85,175,0.4),0px_1px_1px_0px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(255,255,255,1)_inset,0_0_0_1px_rgba(139,92,246,0.32)_inset]",
          isCurrent &&
            "bg-[#d8ccf6] shadow-[0px_0px_1px_0px_rgba(71,52,113,0.45),0px_1px_1px_0px_rgba(71,52,113,0.12),0px_1px_2px_0px_rgba(78,55,130,0.22)_inset,0_0_0_1px_rgba(124,58,237,0.5)_inset]",
          isError &&
            "bg-[#fee2e2] shadow-[0px_0px_1px_0px_rgba(127,29,29,0.45),0px_1px_1px_0px_rgba(127,29,29,0.12),0px_1px_2px_0px_rgba(127,29,29,0.18)_inset]",
          isPressed &&
            "bg-[#d8ccf6] shadow-[0px_0px_1px_0px_rgba(71,52,113,0.45),0px_1px_1px_0px_rgba(71,52,113,0.12),0px_1px_2px_0px_rgba(78,55,130,0.22)_inset]",
          className,
        )}
      >
        <div
          className={cn(
            "flex h-full w-full flex-col items-center justify-center text-[6px] text-neutral-700",
            (isTarget || isCurrent || isPressed) && "text-[#43335f]",
            isError && "text-[#7f1d1d]",
            childrenClassName,
          )}
        >
